var dan = (function () {
    ...
    function register (endpoint, profile, callback) {
        ...
        function register_callback (result, d_name, password = '') {
            if (result) {
                if (!_registered) { ////沒被register過的話 就進去這個if
                    _registered = true;
                    _password = password;
    ...
    function pull_ctl () {
        ...
        function pull_ctl_callback (dataset, error) { ////出錯時(?)
            if (has_new_data(dataset, _ctl_timestamp)) {
                _ctl_timestamp = dataset[0][0];
                if (handle_command_message(dataset[0][1])) { ////可能其實是command
    ...
    function pull_odf (index) {
        ...
        if (!_df_selected[_odf_name]) {  ////_df_selected[_odf_name]有的話就跳過這個if
            pull_odf(index + 1);
            return;
        }

        function pull_odf_callback (dataset, error) { ////出錯時
