...
// VVVVVV CHANGE THESE TWO VVVVVVVVVV
// simulation mode 1: after making a move, you are forced to stop and wait ${interval} seconds before you can move again
// simulation mode 2: after making a move, you keep moving in the same direction until ${interval} seconds later, then you will stop and can issue another move
var sim_mode = 2;
var interval = 300; // simulate data interarrival time: after a move is made, this many milliseconds must pass before you can move again
// ^^^^^^ CHANGE THESE TWO ^^^^^^^^^^
var t = Date.now(); // variable to record time
var momentum = 0; // 0 = stop, 1 = right, -1 = left
...
function updatePlayer () {
    switch(sim_mode) {
        case 1: // stutter steps
            if(keyboard.left.isDown && Date.now()-t > interval) {
                player.body.velocity.x = -250;
                t = Date.now();
            } else if(keyboard.right.isDown && Date.now()-t > interval) {
                player.body.velocity.x = 250;
                t = Date.now();
            } else {
